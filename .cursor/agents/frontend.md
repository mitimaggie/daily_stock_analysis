...
- **桌面优先**：用户使用 MacBook Air 13.3 寸（1440×900），浏览器全屏或与券商软件分屏（约 720px）。所有页面必须在 720px-1440px 范围内有良好体验。不需要适配手机
- **避免信息过载**：散户不是专业分析师，复杂指标要有通俗解释或折叠展示
- **操作确认**：涉及持仓变动的操作（加仓、减仓、清仓）要有二次确认
- **跨页数据一致性**：同一只股票在不同页面展示的数据（评分、止损价、盈亏等）必须一致。如果发现不一致，必须立即向研发总监报告，不可标注为"非本次范围"跳过
