...
const priorityConfig: Record<TodoItem['priority'], { dot: string; border: string }> = {
  ...
  low: { dot: 'bg-white/20', border: 'border-l-white/10' },
};
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[13px] font-mono text-white/80">{item.code}</span>
                  <span className="text-[12px] text-secondary">{item.name}</span>
                </div>
                <p className="text-[12px] text-white/70 pl-4">{item.message}</p>
                {item.detail && (
