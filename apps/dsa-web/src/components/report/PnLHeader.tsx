  ...
  return (
    <div className="rounded-xl bg-[var(--bg-card)] border border-white/[0.06] p-4">
      ...
      <div className="flex items-end gap-3 flex-wrap">
        {pnlPct != null && (
          <div>
            <div className="text-[11px] text-white/30 mb-0.5">浮盈亏</div>
        ...
        <div className="flex items-center gap-3 text-[12px] text-white/45 ml-auto">
          {holdingDays != null && (
            <span>持有 <span className="text-white/65 font-mono">{holdingDays}</span> 天</span>
          )}
          {cost > 0 && (
            <span>成本 <span className="text-white/65 font-mono">{cost.toFixed(2)}</span></span>
          )}
          {shares != null && shares > 0 && (
            <span className="text-white/35 font-mono">{shares}股</span>
          )}
        </div>
      </div>
    </div>
