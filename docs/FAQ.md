...
**现象**：日志里反复出现 `筹码分布获取失败 XXXX: ('Connection aborted.', RemoteDisconnected(...))`

**原因**：筹码数据来自东方财富（通过 AkShare 的 `stock_cyq_em`），该接口容易被对方断开或限流，属于数据源侧不稳定。

**解决方案**：
1. **不需要筹码分析时**：在 `.env` 中设置 `ENABLE_CHIP_DISTRIBUTION=false`，关闭后不再请求筹码接口，也不会再报该警告。
2. 失败时已降为 **debug** 级别日志，默认不刷屏；需要排查时可开启 DEBUG 查看。
3. 核心分析（技术面、F10、舆情）不依赖筹码，关闭后不影响主流程。
4. **需要筹码但不想在分析时拉取**：用「定时拉取 + 缓存」：每天固定时间跑 `python main.py --chip-only` 拉取筹码落库，分析时用缓存。见 Q4.3。

---

### Q4.3: 如何「每天固定时间拉取筹码、分析时只用缓存」？

**做法**：
1. **分析时只用缓存**：在 `.env` 中设置：
   ```bash
   ENABLE_CHIP_DISTRIBUTION=true
   CHIP_FETCH_ONLY_FROM_CACHE=true
   CHIP_CACHE_HOURS=24
   ```
   - `CHIP_CACHE_HOURS=24`：24 小时内都使用已落库的筹码数据，不重新请求接口。
   - `CHIP_FETCH_ONLY_FROM_CACHE=true`：只读缓存，不实时拉取（避免分析时触发接口失败）。
2. **每天固定时间自动拉取筹码**（二选一）：
   - **推荐：用内置定时**：启用定时任务后，脚本会**自动**在每天 `CHIP_SCHEDULE_TIME`（默认 **16:00**，收盘后）跑一次筹码拉取，在 `SCHEDULE_TIME`（默认 18:00）跑全量分析。在 `.env` 中设置：
     ```bash
     SCHEDULE_ENABLED=true
     SCHEDULE_TIME=18:00
     CHIP_SCHEDULE_TIME=16:00
     ```
     然后前台或后台运行一次：
     ```bash
     python main.py --schedule
     ```
     之后每天 16:00 自动拉取筹码、18:00 自动分析/推送，无需 crontab。
   - **或用手动/crontab**：在固定时间执行一次 `python main.py --chip-only`（会按自选股逐个拉取并写入表 `chip_cache`，每只间隔 2 秒防封）；或用系统 crontab 每天跑一次该命令。

---

### Q4.2: 「保存 XXX 数据成功，新增 0 条」是啥意思？

**现象**：日志显示 `保存 002594 数据成功，新增 0 条`

**原因**：日线是按「股票代码 + 日期」写入数据库的。若该股票在这些日期**已有记录**，本次会做**更新**而不是插入，所以「新增」条数为 0。同一天多次跑、或历史已落过库时都会出现。

**结论**：属于正常情况，表示数据已成功写入/更新，并非保存失败。
