# -*- coding: utf-8 -*-
"""
止损策略对比回测：技术止损 vs 成本线止损

回测逻辑：
1. 从历史 K 线中，在每个时间点模拟「买入」（成本价 = 买入当日收盘价）
2. 技术止损：基于 ATR 动态计算 stop_loss_short
3. 成本线止损：跌破买入成本价即止损
4. 两种策略各自持仓 20 个交易日（约 1 个月），统计结果
...
COST_OFFSET_PCT = 0.0   # 成本价 = 买入收盘价（不做偏移，模拟当日买入）

# ATR 参数（与 risk_management.py 一致）
