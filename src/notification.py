...
class NotificationService:
    ...
    def generate_daily_report(
        ...
        """
        生成 Markdown 格式的日报（统一使用 dashboard 格式）
        ...
        Returns:
            Markdown 格式的日报内容
        """
        # 统一调用 generate_dashboard_report，避免重复推送
        return self.generate_dashboard_report(results, report_date)
        ...
        for result in sorted_results:
            ...
            if intel:
                ...
                if intel.get('earnings_outlook'):
                    intel_items.append(f"📊 **业绩预期**: {intel['earnings_outlook']}")
                if intel.get('sentiment_summary'):
                    intel_items.append(f"💭 **舆情情绪**: {intel['sentiment_summary']}")
            ...
            if has_battle or sl > 0 or tp_short > 0:
                ...
                rr_val = f"{rr:.1f}:1" if rr > 0 else "N/A"
                report_lines.extend([
                ...
                report_lines.extend([
                    "### 🏦 持仓建议",
                    "",
                    f"- 🆕 **空仓者**: {advice_empty}" + (f" (仓位≤{pos_pct}%)" if pos_pct > 0 else ""),
                    f"- 💼 **持仓者**: {advice_hold}",
                    "",
                ])
