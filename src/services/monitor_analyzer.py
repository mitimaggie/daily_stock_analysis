...
def _run_monitor_flash(
    ...
    try:
        ...
        if not resp.success:
            logger.debug("[monitor_analyzer] Flash诊断失败: %s", resp.error)
            return None
        text = resp.text.strip()
        return text[:800] if text else None
    except Exception as e:
        logger.debug("[monitor_analyzer] Flash诊断失败: %s", e)
        return None
...
{{"decision": "A/B/C", "action_price": 数字或null, "new_stop": 数字或null, "next_checkpoint": "价格或日期", "reasoning": "≤60字"}}"""
        ...
        if not resp.success:
            logger.debug("[monitor_analyzer] Pro诊断失败: %s", resp.error)
            return None
        text = resp.text.strip()
        ...
        logger.debug("[monitor_analyzer] Pro诊断失败: %s", e)
        return None
